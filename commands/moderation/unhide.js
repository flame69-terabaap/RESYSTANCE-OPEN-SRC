...
module.exports = {
    ...
    description: "Unhides a channel for everyone",
    cooldown: 5,
                ...
                message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(`${client.emoji.tick} | Successfully unhidden \`${channel.name}\` for @everyone role.`)
                    ]
                });
            }).catch(() => {
                message.channel.send(`${client.emoji.cross} | Failed to unhide \`${channel.name}\`.`);
            });
        } else {
            message.channel.send(`${client.emoji.cross} | I don't have permission to unhide \`${channel.name}\`.`);
        }
    }
}