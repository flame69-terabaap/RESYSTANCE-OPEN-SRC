...
module.exports = {
    name: "vcdeafen",
    aliases: ["vcdeaf", "voicedeafen"],
    category: "voice",
    description: "Deafens a member present in the voice channel.",
    usage: "?vcdeafen <@user>",
    cooldown: 5,
    ...
    run: async (client, message, args, prefix) => {
        ...
        if (member.voice.serverDeaf === true)
            return message.channel.send(`${client.emoji.cross} | The mentioned member is already deafened in the voice channel.`);

        member.voice.setDeaf(true);
        message.channel.send(`${client.emoji.tick} | Successfully deafened ${member.user.username} in the voice channel.`);
    }
};